...
export const Progress = ({ progress, color }) => (
    <div className={styles.progressWrapper}>
        <div className={styles.progressBar(color)} style={{ width: `${Math.min(Math.max(progress * 20, 0), 100)}%`, backgroundColor: color }} />
    </div>
);
...
const InfoBlock = ({ label, progress, accentColor }) => (
    <div className={styles.infoRow}>
        ...
        {progress > 0 && <Progress progress={Math.min((progress / 100) * 5, 5)} color={accentColor} />}
    </div>
);
        ...
        <div className={styles.projectLinks}>
            {githubLink && (
                <a href={githubLink} target="_blank" rel="noopener noreferrer" className={styles.linkRow}>
                    <span className="text-current"><Github size={16} /></span><span>GitHub</span>
                </a>
            )}
            {liveDemoUrl && (
                <a href={liveDemoUrl} target="_blank" rel="noopener noreferrer" className={styles.linkRow}>
                    <span className="text-current"><ExternalLink size={16} /></span><span>Live Demo</span>
                </a>
            )}
        </div>
    </div>
...
export const RatingInput = ({ value = 0, total = 5, onChange = () => { }, color = '#10b981', bgColor = '#e5e7eb' }) => {
    const displayValue = Math.round((Math.min(Math.max(value, 0), 100) / 100) * total);
    return (
